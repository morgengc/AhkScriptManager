...
#else
            register_hotkey(i*2, MOD_ALT, i+1+0x30);
            register_hotkey(i*2+1, MOD_CONTROL, i+1+0x30);
#endif
		}
	} else {
		for(i=0; i<NUM_DESKTOPS; i++) {
...
#else
    register_hotkey(i*2, MOD_ALT|MOD_CONTROL|MOD_SHIFT, 'Q');
    register_hotkey(i*2+1, MOD_ALT|MOD_CONTROL|MOD_SHIFT, 'S');
#endif
